...
// this sketch uses openCV to create a simply falling star effect playable game
...
void draw () {
  ...
  for (int i = 0; i < stars.size(); i++) { // for every star in the star array
    Star _star = (Star) stars.get(i); // put the current star into a temp object
